...
int main(int argc, char *argv[])
{
	...
	csvfile << "time Dxx Dxy Dxz Dyx Dyy Dyz Dzx Dzy Dzz\n";
       ...
       {
            Info << "No tensor D found — using isotropic DT * I\n" << endl;
       }
       ...
       {	
		...
		if (hasTensorD || hasTensorD0)
		{
		    volTensorField gradb ("gradb", (*DPtr) & fvc::grad(b));
		    Deff = sum((*DPtr)*eps*clip)/poro/sum(clip) - sum(Utildab*clip)/poro/sum(clip) + sum(gradb*eps*clip)/poro/sum(clip);
                }
		else
		{
		    volTensorField gradb ("gradb", DT * fvc::grad(b));
                    Deff = sum(DT*eps*clip)/poro/sum(clip)*tensor(I) - sum(Utildab*clip)/poro/sum(clip) + sum(gradb*eps*clip)/poro/sum(clip);
		}
