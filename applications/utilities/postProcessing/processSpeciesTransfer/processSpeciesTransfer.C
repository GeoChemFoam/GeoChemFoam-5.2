...
int main(int argc, char *argv[])
{
	...
	csvfile << "time Tf Tm coeff\n";
       ...
       {	
		...
		gradT = gradT + (fvc::snGrad(T)-(gradT & nf))*nf;
            
		surfaceVectorField flux = fvc::interpolate(eps)*DT*gradT;
		if (hasTensorD || hasTensorD0) flux = fvc::interpolate(eps*(*DPtr)) & gradT;
                surfaceScalarField phiD = flux & mesh.Sf();
