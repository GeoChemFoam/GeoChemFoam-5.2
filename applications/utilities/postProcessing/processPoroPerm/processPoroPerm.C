...
int main(int argc, char *argv[])
{
	...
	csvfile << "time poro perm Lpore Re UD tau\n";
	...
	forAll(timeList, timeStep)
	{
        ...
        word lengthScale
        {
            postProcessDict.lookupOrDefault<word>("lengthScale","BundleOfTubes")
        };
		...
		scalar tau = magU.weightedAverage(clip).value()/(Uavg+1e-30);
		...
		scalar Lpore = std::sqrt(8*perm/poro);
                if (lengthScale=="BundleOfTubes")
                {
                   //nothing to do
                }
                else if (lengthScale=="BundleOfTubes2D")
                {
                   Lpore = std::sqrt(12*perm/poro);
                }
                else if (lengthScale=="KozenyCarman")
                {
                   Lpore = tau*std::sqrt(72*perm/poro);
                }
                else if (lengthScale=="KozenyCarman2D")
                {
                   Lpore = tau*std::sqrt(32*perm/poro);
                }

                else if (lengthScale=="direct")
                {
                   Lpore = readScalar(postProcessDict.lookup("Lref"));
                }
                else
                {
                   Info<< "lengthScale should be equal to direct or BundleOfTubes or BundleOfTubes2D or KozenyCarman"
                    << endl
                   << abort(FatalError);
                }
		...
		if (Pstream::master())
		{
			...
			csvfile << runTime.timeName() << " " << poro << " " << perm << " " << Lpore << " " << Re << " " << UD << " " << tau << "\n";
		}
	}
