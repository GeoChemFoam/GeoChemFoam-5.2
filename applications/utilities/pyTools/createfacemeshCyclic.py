...
def main(xDim, yDim, zDim, xMin, xMax, yMin, yMax, zMin, zMax, nX, nY, nZ, padWidth, direction, NPX, NPY, NPZ, rank, output_path):
 ...
 if (ipz==NPZ-1) and (NPZ>2):
   ##proc rank to rank-NPX*NPY*(NPZ-1) through back boundary
   for i in range(0,nxp):
     for j in range(0,nyp):
       ##bottom face of (ith,kth) column
       x1=nzp*(nxp+1)*(nyp+1)+j*(nxp+1)+i
 ...
 if (ipy==NPY-1) and (NPY>2):
   ##proc rank to rank-NPX*(NPY-1) through top boundary
   for i in range(0,nxp):
     for k in range(0,nzp):
       ##bottom face of (ith,kth) column
       x1=k*(nxp+1)*(nyp+1)+nyp*(nxp+1)+i
 ...
 if (ipx==NPX-1) and (NPX>2):
   ##proc 0 to rank-NPX+1 through right boundary
   for k in range(0,nzp):
     for j in range(0,nyp):
       ##left face of (jth,kth) column
       x1=k*(nxp+1)*(nyp+1)+j*(nxp+1)+nxp
 ...
 if (ipy==0) and (NPY>1):
   ##proc rank to rank+NPX*(NPY-1) through bottom boundary
   for i in range(0,nxp):
     for k in range(0,nzp):
     ##bottom face of (ith,kth) column
       x1=k*(nxp+1)*(nyp+1)+i
 ...
 if (ipz==0) and (NPZ>1):
   ##proc rank to rank+NPX*NPY*(NPZ-1) through front boundary
   for i in range(0,nxp):
     for j in range(0,nyp):
       ##bottom face of (ith,kth) column
       x1=j*(nxp+1)+i
 ...
 if NPX*NPY*NPZ>1:
   ...
   if (ipz==NPZ-1) and (NPZ>2):
     ##rank to rank - NPX*NPY*(NPZ-1) though back boundary
     for i in range(0,nxp):
       for j in range(0,nyp):
         data.append(f"{nx1*ny1*(nz1-1)+nx1*(ny1-1)*nz1+(nx1-1)*ny1*nz1+2*ny1*nz1+2*nx1*nz1+nx1*ny1+(i+startX)*ny1+j+startY+1}\n")
   ...
   if (ipx==NPX-1) and (NPX>2):
     ##proc NPX-1 to 0 through right boundary
     for k in range(0,nzp):
       for j in range(0,nyp):
         data.append(f"{nx1*ny1*(nz1-1)+nx1*(ny1-1)*nz1+(nx1-1)*ny1*nz1+ny1*nz1+(k+startZ)*ny1+j+startY+1}\n")
   ...
   if (ipx==0) and (NPX>1):
     ##proc rank to ran+NPX-1 through left boundary
     for k in range(0,nzp):
       for j in range(0,nyp):
         data.append(f"{nx1*ny1*(nz1-1)+nx1*(ny1-1)*nz1+(nx1-1)*ny1*nz1+(k+startZ)*ny1+j+startY+1}\n")
   ...
   if (ipz==0) and (NPZ>1):
     ##proc rank to rank+NPX*NPY*(NPZ-1) front boundary
     for i in range(0,nxp):
       for j in range(0,nyp):
         data.append(f"{nx1*ny1*(nz1-1)+nx1*(ny1-1)*nz1+(nx1-1)*ny1*nz1+2*ny1*nz1+2*nx1*nz1+(i+startX)*ny1+j+startY+1}\n")
 ...
 if (ipz == NPZ - 1) and (NPZ>2):
   ##proc rank to rank-NPX*NPY*(NPZ-1) back boundary
   for i in range(nxp):
     for j in range(nyp):
       data.append(f"{(nzp - 1) * nxp * nyp + j * nxp + i}\n")
 ...
 if (ipx == NPX - 1) and (NPX>2):
   ##proc rank to rank-NPX+1 through right boundary
   for k in range(nzp):
     for j in range(nyp):
       data.append(f"{k * nxp * nyp + j * nxp + nxp - 1}\n")
 ...
 if (ipx == 0) and (NPX>1):
   ##proc rank to rank+NPX-1 through Left boundary
   for k in range(nzp):
     for j in range(nyp):
       data.append(f"{k * nxp * nyp + j * nxp}\n")
 if (ipy < NPY - 1):
