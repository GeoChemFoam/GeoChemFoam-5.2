...
def main(xDim, yDim, zDim, xMin, xMax, yMin, yMax, zMin, zMax, nX, nY, nZ, res,Image_name, padWidth, pad_value, value1,value0, field_min,dimension,direction, nlevel,NPX, NPY, NPZ, rank, output_path,field_name):
 ...
 for global_layer in range(0,zmax-zmin):
       ...
       if (0 <= global_layer < r1):
         ...
         if global_layer < pad_in_z or global_layer > zMax-zMin+pad_in_z-1:
           ...
           if direction==0:
             ...
             my_array = np.pad(my_array,pad_width=((0,0),(padWidth,padWidth)),mode='constant',constant_values=value1)
           elif direction==1:
             #add pad in direction 1
             my_array = np.pad(my_array,pad_width=((padWidth,padWidth),(0,0)),mode='constant',constant_values=value1)
           elif direction==-1:
             #add pad in direction 0
             my_array = np.pad(my_array,pad_width=((0,0),(padWidth,padWidth)),mode='constant',constant_values=value1)
             #add pad in direction 1
   ...
   if (ipz==0) and (NPZ>1):
       if (zmax-zmin-r1 <= global_layer < zmax-zmin):
   ...
   if startZ*r <= global_layer < (startZ+nzp)*r:
     ...
     if (ipy==NPY-1) and (NPY>1):
       my_array_3d = np.reshape(my_array[0:q1,startX*p:(startX+nxp)*p], (1,q1,nxp*p))
       if global_layer == startZ*r:
     ...
     if (ipx==NPX-1) and (NPX>1):
       my_array_3d = np.reshape(my_array[startY*q:(startY+nyp)*q,0:p1], (1,nyp*q,p1))
       if global_layer == startZ*r:
     ...
     if (ipx==0) and (NPX>1):
       my_array_3d = np.reshape(my_array[startY*q:(startY+nyp)*q,xmax-xmin-p1:xmax-xmin], (1,nyp*q,p1))
       if global_layer == startZ*r:
     ...
     if (ipy==0) and (NPY>1):
       my_array_3d = np.reshape(my_array[ymax-ymin-q1:ymax-ymin,startX*p:(startX+nxp)*p], (1,q1,nxp*p))
       if global_layer == startZ*r:
 ...
 if NP>1:
   ...
   output_path = ""
 with open(output_path+"0/"+field_name, 'r') as f:
     lines = f.readlines()
 ...
 for i, line in enumerate(lines):
     if 'internalField' in line and 'nonuniform' in line:
         ...
         for j in range(n):
             field[j] = float(lines[start_idx + j].strip())
         i=start_idx+n
         break
         ...
         if f"procBoundary{rank}to{rank-NPX*NPY*(NPZ-1)}throughback" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bz01 = np.empty(nbz01, dtype=np.float64)
                     for j in range(nbz01):
                         field_bz01[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bz01 = np.empty(nbz01, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bz01[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipz01==1):
         ...
         if f"procBoundary{rank}to{rank-NPX*NPY}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bz0 = np.empty(nbz0, dtype=np.float64)
                     for j in range(nbz0):
                         field_bz0[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bz0 = np.empty(nbz0, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bz0[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipz0==1):
         ...
         if f"procBoundary{rank}to{rank-NPX*(NPY-1)}throughtop" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_by01 = np.empty(nby01, dtype=np.float64)
                     for j in range(nby01):
                         field_by01[j] = float(lines[start_idx + j].strip())
                     ...
                     field_by01 = np.empty(nby01, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_by01[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipy01==1):
         ...
         if f"procBoundary{rank}to{rank-NPX}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_by0 = np.empty(nby0, dtype=np.float64)
                     for j in range(nby0):
                         field_by0[j] = float(lines[start_idx + j].strip())
                     ...
                     field_by0 = np.empty(nby0, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_by0[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipy0==1):
         ...
         if f"procBoundary{rank}to{rank-NPX+1}throughright" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bx01 = np.empty(nbx01, dtype=np.float64)
                     for j in range(nbx01):
                         field_bx01[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bx01 = np.empty(nbx01, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bx01[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipx01==1):
         ...
         if f"procBoundary{rank}to{rank-1}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bx0 = np.empty(nbx0, dtype=np.float64)
                     for j in range(nbx0):
                         field_bx0[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bx0 = np.empty(nbx0, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bx0[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipx0==1):
         ...
         if f"procBoundary{rank}to{rank+1}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bx1 = np.empty(nbx1, dtype=np.float64)
                     for j in range(nbx1):
                         field_bx1[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bx1 = np.empty(nbx1, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bx1[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipx1==1):
         ...
         if f"procBoundary{rank}to{rank+NPX-1}throughleft" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bx10 = np.empty(nbx10, dtype=np.float64)
                     for j in range(nbx10):
                         field_bx10[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bx10 = np.empty(nbx10, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bx10[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipx10==1):
         ...
         if f"procBoundary{rank}to{rank+NPX}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_by1 = np.empty(nby1, dtype=np.float64)
                     for j in range(nby1):
                         field_by1[j] = float(lines[start_idx + j].strip())
                     ...
                     field_by1 = np.empty(nby1, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_by1[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipy1==1):
         ...
         if f"procBoundary{rank}to{rank+NPX*(NPY-1)}throughbottom" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_by10 = np.empty(nby10, dtype=np.float64)
                     for j in range(nby10):
                         field_by10[j] = float(lines[start_idx + j].strip())
                     ...
                     field_by10 = np.empty(nby10, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_by10[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipy10==1):
         ...
         if f"procBoundary{rank}to{rank+NPX*NPY}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bz1 = np.empty(nbz1, dtype=np.float64)
                     for j in range(nbz1):
                         field_bz1[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bz1 = np.empty(nbz1, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bz1[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipz1==1):
         ...
         if f"procBoundary{rank}to{rank+NPX*NPY*(NPZ-1)}throughfront" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i] and 'nonuniform' in lines[i]:
                     ...
                     start_idx = i + 3
                     field_bz10 = np.empty(nbz10, dtype=np.float64)
                     for j in range(nbz10):
                         field_bz10[j] = float(lines[start_idx + j].strip())
                     ...
                     field_bz10 = np.empty(nbz10, dtype=np.float64)
                     parts = lines[i].replace(';', '').split()
                     field_bz10[0] = float(parts[-1].rstrip(';'))
                     break
                 i += 1
         i +=1
         if (found_ipz10==1):
 ...
 for i, line in enumerate(lines):
     if 'internalField' in line:
         ...
         if (uniform==1):
             uniform=0
             fieldVal=field[0]
             field=np.full(n,fieldVal,dtype=np.float64)
         ...
         if f"procBoundary{rank}to{rank-NPX*NPY*(NPZ-1)}throughback" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bz01==1):
                         uniform_bz01=0
                         fieldVal=field_bz01[0]
                         field_bz01=np.full(nbz01,fieldVal,dtype=np.float64)
                     start_idx = i + 3
                     ix_bz01 = np.empty(nbz01, dtype=np.int64)
                     iy_bz01 = np.empty(nbz01, dtype=np.int64)
                     for j in range(nbz01):
                         vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                         parts = vec_line.split()
                         x, y, z = map(float, parts)
                         ix_bz01[j]=np.floor((x-res*(xmin+startX*p))/dx);
                         iy_bz01[j]=np.floor((y-res*(ymin+startY*q))/dy);
                     i = start_idx + nbz01  # skip ahead
                     break
                 i += 1
         i +=1
         ...
         if f"procBoundary{rank}to{rank-NPX*NPY}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bz0==1):
                         uniform_bz0=0
                         fieldVal=field_bz0[0]
                         field_bz0=np.full(nbz0,fieldVal,dtype=np.float64)
                     start_idx = i + 3
         ...
         if f"procBoundary{rank}to{rank-NPX*(NPY-1)}throughtop" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_by01==1):
                         uniform_by01=0
                         fieldVal=field_by01[0]
                         field_by01=np.full(nby01,fieldVal,dtype=np.float64)
                     start_idx = i + 3
                     ix_by01 = np.empty(nby01, dtype=np.int64)
                     iz_by01 = np.empty(nby01, dtype=np.int64)
                     for j in range(nby01):
                         vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                         parts = vec_line.split()
                         x, y, z = map(float, parts)
                         ix_by01[j]=np.floor((x-res*(xmin+startX*p))/dx);
                         iz_by01[j]=np.floor((z-res*(zmin+startZ*r))/dz);
                     i = start_idx + nby01  # skip ahead
                     break
                 i += 1
         i +=1
         ...
         if f"procBoundary{rank}to{rank-NPX}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_by0==1):
                         uniform_by0=0
                         fieldVal=field_by0[0]
                         field_by0=np.full(nby0,fieldVal,dtype=np.float64)
                     start_idx = i + 3
         ...
         if f"procBoundary{rank}to{rank-NPX+1}throughright" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bx01==1):
                         uniform_bx01=0
                         fieldVal=field_bx01[0]
                         field_bx01=np.full(nbx01,fieldVal,dtype=np.float64)
                     start_idx = i + 3
                     iy_bx01 = np.empty(nbx01, dtype=np.int64)
                     iz_bx01 = np.empty(nbx01, dtype=np.int64)
                     for j in range(nbx01):
                         vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                         parts = vec_line.split()
                         x, y, z = map(float, parts)
                         iy_bx01[j]=np.floor((y-res*(ymin+startY*q))/dy);
                         iz_bx01[j]=np.floor((z-res*(zmin+startZ*r))/dz);
                     i = start_idx + nbx01  # skip ahead
                     break
                 i += 1
         i +=1
         ...
         if f"procBoundary{rank}to{rank-1}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bx0==1):
                         uniform_bx0=0
                         fieldVal=field_bx0[0]
                         field_bx0=np.full(nbx0,fieldVal,dtype=np.float64)
                     start_idx = i + 3
         ...
         if f"procBoundary{rank}to{rank+1}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bx1==1):
                         uniform_bx1=0
                         fieldVal=field_bx1[0]
                         field_bx1=np.full(nbx1,fieldVal,dtype=np.float64)
                     start_idx = i + 3
         ...
         if f"procBoundary{rank}to{rank+NPX-1}throughleft" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bx10==1):
                         uniform_bx10=0
                         fieldVal=field_bx10[0]
                         field_bx10=np.full(nbx10,fieldVal,dtype=np.float64)
                     start_idx = i + 3
                     iy_bx10 = np.empty(nbx10, dtype=np.int64)
                     iz_bx10 = np.empty(nbx10, dtype=np.int64)
                     for j in range(nbx10):
                         vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                         parts = vec_line.split()
                         x, y, z = map(float, parts)
                         iy_bx10[j]=np.floor((y-res*(ymin+startY*q))/dy);
                         iz_bx10[j]=np.floor((z-res*(zmin+startZ*q))/dz);
                     i = start_idx + nbx10  # skip ahead
                     break
                 i += 1
         i +=1
         ...
         if f"procBoundary{rank}to{rank+NPX}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_by1==1):
                         uniform_by1=0
                         fieldVal=field_by1[0]
                         field_by1=np.full(nby1,fieldVal,dtype=np.float64)
                     start_idx = i + 3
         ...
         if f"procBoundary{rank}to{rank+NPX*(NPY-1)}throughbottom" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_by10==1):
                         uniform_by10=0
                         fieldVal=field_by10[0]
                         field_by10=np.full(nby10,fieldVal,dtype=np.float64)
                     start_idx = i + 3
                     ix_by10 = np.empty(nby10, dtype=np.int64)
                     iz_by10 = np.empty(nby10, dtype=np.int64)
                     for j in range(nby10):
                         vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                         parts = vec_line.split()
                         x, y, z = map(float, parts)
                         ix_by10[j]=np.floor((x-res*(xmin+startX*p))/dx);
                         iz_by10[j]=np.floor((z-res*(zmin+startZ*r))/dy);
                     i = start_idx + nby10  # skip ahead
                     break
                 i += 1
         i +=1
         ...
         if f"procBoundary{rank}to{rank+NPX*NPY}" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bz1==1):
                         uniform_bz1=0
                         fieldVal=field_bz1[0]
                         field_bz1=np.full(nbz1,fieldVal,dtype=np.float64)
                     start_idx = i + 3
         ...
         if f"procBoundary{rank}to{rank+NPX*NPY*(NPZ-1)}throughfront" in line:
             ...
             while (i<len(lines)):
                 if 'value' in lines[i]:
                     ...
                     if (uniform_bz10==1):
                         uniform_bz10=0
                         fieldVal=field_bz10[0]
                         field_bz10=np.full(nbz10,fieldVal,dtype=np.float64)
                     start_idx = i + 3
                     ix_bz10 = np.empty(nbz10, dtype=np.int64)
                     iy_bz10 = np.empty(nbz10, dtype=np.int64)
                     for j in range(nbz10):
                         vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                         parts = vec_line.split()
                         x, y, z = map(float, parts)
                         ix_bz10[j]=np.floor((x-res*(xmin+startX*p))/dx);
                         iy_bz10[j]=np.floor((y-res*(ymin+startY*q))/dy);
                     i = start_idx + nbz10  # skip ahead
                     break
                 i += 1
         i +=1
 ...
 #field=np.zeros((nxp,nyp,nzp),dtype="float64")
 for j in range(0,n):
    fieldVal=field[j]
    if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
      field[j]=0
      for ii in range (0,p1):
        for jj in range (0,q1):
          for kk in range (0,r1):
             field[j]+=((my_array_p[iz[j]*r1+kk,iy[j]*q1+jj,ix[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p[iz[j]*r1+kk,iy[j]*q1+jj,ix[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc k to k-1
 if (ipz>0):
   for j in range(0,nbz0):
     fieldVal=field_bz0[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bz0[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_bz0[j]+=((my_array_p_z0[kk,iy_bz0[j]*q1+jj,ix_bz0[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_z0[kk,iy_bz0[j]*q1+jj,ix_bz0[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc through front
 if (ipz==NPZ-1) and (NPZ>1):
   for j in range(0,nbz01):
     fieldVal=field_bz01[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bz01[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             a=my_array_p_z01[kk,iy_bz01[j]*q1+jj,ix_bz01[j]*p1+ii]
             field_bz01[j]+=((a-value0)/(value1-value0)+ field_min*(value1-a)/(value1-value0))/p1/q1/r1
#proc j to j-1
 if (ipy>0):
   for j in range(0,nby0):
     fieldVal=field_by0[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_by0[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_by0[j]+=((my_array_p_y0[iz_by0[j]*r1+kk,jj,ix_by0[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_y0[iz_by0[j]*r1+kk,jj,ix_by0[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc through top 
 if (ipy==NPY-1) and (NPY>1):
   for j in range(0,nby01):
     fieldVal=field_by01[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_by01[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_by01[j]+=((my_array_p_y01[iz_by01[j]*r1+kk,jj,ix_by01[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_y01[iz_by01[j]*r1+kk,jj,ix_by01[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc i to i-1
 if (ipx>0):
   for j in range(0,nbx0):
     fieldVal=field_bx0[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bx0[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_bx0[j]+=((my_array_p_x0[iz_bx0[j]*r1+kk,iy_bx0[j]*q1+jj,ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_x0[iz_bx0[j]*r1+kk,iy_bx0[j]*q1+jj,ii])/(value1-value0))/p1/q1/r1

 #proc trough right 
 if (ipx==NPX-1) and (NPX>1):
   for j in range(0,nbx01):
     fieldVal=field_bx01[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bx01[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_bx01[j]+=((my_array_p_x01[iz_bx01[j]*r1+kk,iy_bx01[j]*q1+jj,ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_x01[iz_bx01[j]*r1+kk,iy_bx01[j]*q1+jj,ii])/(value1-value0))/p1/q1/r1


 #proc i to i+1
 if (ipx<NPX-1):
   for j in range(0,nbx1):
     fieldVal=field_bx1[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bx1[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_bx1[j]+=((my_array_p_x1[iz_bx1[j]*r1+kk,iy_bx1[j]*q1+jj,ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_x1[iz_bx1[j]*r1+kk,iy_bx1[j]*q1+jj,ii])/(value1-value0))/p1/q1/r1

 #proc trough left 
 if (ipx==0) and (NPX>1):
   for j in range(0,nbx10):
     fieldVal=field_bx10[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bx10[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
             field_bx10[j]+=((my_array_p_x10[iz_bx10[j]*r1+kk,iy_bx10[j]*q1+jj,ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_x10[iz_bx10[j]*r1+kk,iy_bx10[j]*q1+jj,ii])/(value1-value0))/p1/q1/r1


 #proc j to j+1
 if (ipy<NPY-1):
   for j in range(0,nby1):
     fieldVal=field_by1[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_by1[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
               field_by1[j]+=((my_array_p_y1[iz_by1[j]*r1+kk,jj,ix_by1[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_y1[iz_by1[j]*r1+kk,jj,ix_by1[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc through bottom 
 if (ipy==0) and (NPY>1):
   for j in range(0,nby10):
     fieldVal=field_by10[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_by10[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
               field_by10[j]+=((my_array_p_y10[iz_by10[j]*r1+kk,jj,ix_by10[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_y10[iz_by10[j]*r1+kk,jj,ix_by10[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc k to k+1
 if (ipz<NPZ-1):
   for j in range(0,nbz1):
     fieldVal=field_bz1[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bz1[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
               field_bz1[j]+=((my_array_p_z1[kk,iy_bz1[j]*q1+jj,ix_bz1[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_z1[kk,iy_bz1[j]*q1+jj,ix_bz1[j]*p1+ii])/(value1-value0))/p1/q1/r1

 #proc through front
 if (ipz==0) and (NPZ>1):
   for j in range(0,nbz10):
     fieldVal=field_bz10[j]
     if (fieldVal<1-(1-field_min)*5e-3) and (fieldVal> field_min+(1-field_min)*5e-3):
       field_bz10[j]=0
       for ii in range (0,p1):
         for jj in range (0,q1):
           for kk in range (0,r1):
               field_bz10[j]+=((my_array_p_z10[kk,iy_bz10[j]*q1+jj,ix_bz10[j]*p1+ii]-value0)/(value1-value0)+ field_min*(value1-my_array_p_z10[kk,iy_bz10[j]*q1+jj,ix_bz10[j]*p1+ii])/(value1-value0))/p1/q1/r1
 ...
 for j in range(n):
       num_string = format(field[j], ".8f").rstrip('0').rstrip('.')
 ...
 if (ipz==NPZ-1) and (NPZ>2):
   ...
   for j in range(nbz01):
       num_string = format(field_bz01[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipz>0):
   ...
   for j in range(nbz0):
       num_string = format(field_bz0[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipy==NPY-1) and (NPY>2):
   ...
   for j in range(nby01):
       num_string = format(field_by01[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipy>0):
   ...
   for j in range(nby0):
       num_string = format(field_by0[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipx==NPX-1) and (NPX>2):
   ...
   for j in range(nbx01):
       num_string = format(field_bx01[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipx>0):
   ...
   for j in range(nbx0):
       num_string = format(field_bx0[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipx<NPX-1):
   ...
   for j in range(nbx1):
       num_string = format(field_bx1[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipx==0) and (NPX>1):
   ...
   for j in range(nbx10):
       num_string = format(field_bx10[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipy<NPY-1):
   ...
   for j in range(nby1):
       num_string = format(field_by1[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipy==0) and (NPY>1):
   ...
   for j in range(nby10):
       num_string = format(field_by10[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipz<NPZ-1):
   ...
   for j in range(nbz1):
       num_string = format(field_bz1[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 if (ipz==0) and (NPZ>1):
   ...
   for j in range(nbz10):
       num_string = format(field_bz10[j], ".8f").rstrip('0').rstrip('.')
       data.append(f"{num_string}\n")
   data.extend([
   ")\n",
   ";\n",
 ...
 # Write data to file
 with open(output_path+'0/'+field_name, 'w') as f:
   f.writelines(data)
   f.close()
 ...
 #if(rank==0):
 #  print("Elapsed time and io time createfield:", rank, elapsed_time, io_elapsed_time, "seconds")
