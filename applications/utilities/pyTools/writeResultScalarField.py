...
var_local = np.zeros((nxp, nyp, nzp), dtype='f8')

file = open(output_path+"0/"+var,"r")
...
all_var = comm.gather(var_local, root=0)
...
if rank == 0:
    ...
    var_global = np.empty((nx1, ny1, nz1), dtype='f8')

    for r, block in enumerate(all_var):
        # Use startX[r], startY[r], startZ[r] to place block in U_global
        var_global[all_startX[r]:all_startX[r]+all_nxp[r], all_startY[r]:all_startY[r]+all_nyp[r], all_startZ[r]:all_startZ[r]+all_nzp[r]] = block

    with h5py.File("Result.h5", "a") as f:
        if var in f:
            del f[var]  # Delete existing dataset if you want to overwrite it
        f.create_dataset(var, data=var_global)
