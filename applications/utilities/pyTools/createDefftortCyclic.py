...
if (ipz==NPZ-1) and (NPZ>1):
    D_bz01=np.empty(nbz01, dtype=np.float64)
    for j in range(nbz01):
        epsVal=eps_bz01[j]
        lower = upper = None
        for k in range(len(micro_por_array) - 1):
...
if (ipy==NPY-1) and (NPY>1):
    D_by01=np.empty(nby01, dtype=np.float64)
    for j in range(nby01):
        epsVal=eps_by01[j]
        lower = upper = None
        for k in range(len(micro_por_array) - 1):
...
if (ipx==NPX-1) and (NPX>1):
    D_bx01=np.empty(nbx01, dtype=np.float64)
    for j in range(nbx01):
        epsVal=eps_bx01[j]
        lower = upper = None
        for k in range(len(micro_por_array) - 1):
...
if (ipx==0) and (NPX>1):
    D_bx10=np.empty(nbx10, dtype=np.float64)
    for j in range(nbx10):
        epsVal=eps_bx10[j]
        lower = upper = None
        for k in range(len(micro_por_array) - 1):
...
if (ipy==0) and (NPY>1):
    D_by10=np.empty(nby10, dtype=np.float64)
    for j in range(nby10):
        epsVal=eps_by10[j]
        lower = upper = None
        for k in range(len(micro_por_array) - 1):
...
if (ipz==0) and (NPZ>1):
    D_bz10=np.empty(nbz10, dtype=np.float64)
    for j in range(nbz10):
        epsVal=eps_bz10[j]
        lower = upper = None
        for k in range(len(micro_por_array) - 1):
