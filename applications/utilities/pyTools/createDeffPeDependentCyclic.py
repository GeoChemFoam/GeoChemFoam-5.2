            ...
            while (i<len(lines)):
                if 'value' in lines[i] and 'nonuniform' in lines[i]:
                    ...
                    if (uniform_bz01==1):
                        uniform_bz01=0
                        epsVal=eps_bz01[0]
                        eps_bz01=np.full(nbz01,epsVal,dtype=np.float64)
                    start_idx = i + 3
                    U_bz01 = np.empty(nbz01, dtype=np.float64)
                    for j in range(nbz01):
                        vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                        parts = vec_line.split()
                        Ux, Uy, Uz = map(float, parts)
                        U_bz01[j]=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    i = start_idx + nbz01  # skip ahead
                    break
                elif 'value' in lines[i] and 'uniform' in lines[i]:
                    parts = lines[i].replace(';', '').replace('(', '').replace(')', '').split()
                    ...
                    Uz = float(parts[-1])
                    magU=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    U_bz01=np.full(nbz01,magU,dtype=np.float64)
                    break
                i += 1
        i +=1
        if (found_ipz01==1):
            ...
            while (i<len(lines)):
                if 'value' in lines[i] and 'nonuniform' in lines[i]:
                    ...
                    if (uniform_by01==1):
                        uniform_by01=0
                        epsVal=eps_by01[0]
                        eps_by01=np.full(nby01,epsVal,dtype=np.float64)
                    start_idx = i + 3
                    U_by01 = np.empty(nby01, dtype=np.float64)
                    for j in range(nby01):
                        vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                        parts = vec_line.split()
                        Ux, Uy, Uz = map(float, parts)
                        U_by01[j]=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    i = start_idx + nby01  # skip ahead
                    break
                elif 'value' in lines[i] and 'uniform' in lines[i]:
                    parts = lines[i].replace(';', '').replace('(', '').replace(')', '').split()
                    ...
                    Uz = float(parts[-1])
                    magU=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    U_by01=np.full(nby01,magU,dtype=np.float64)
                    break
                i += 1
        i +=1
        if (found_ipy01==1):
            ...
            while (i<len(lines)):
                if 'value' in lines[i] and 'nonuniform' in lines[i]:
                    ...
                    if (uniform_bx01==1):
                        uniform_bx01=0
                        epsVal=eps_bx01[0]
                        eps_bx01=np.full(nbx01,epsVal,dtype=np.float64)
                    start_idx = i + 3
                    U_bx01 = np.empty(nbx01, dtype=np.float64)
                    for j in range(nbx01):
                        vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                        parts = vec_line.split()
                        Ux, Uy, Uz = map(float, parts)
                        U_bx01[j]=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    i = start_idx + nbx01  # skip ahead
                    break
                elif 'value' in lines[i] and 'uniform' in lines[i]:
                    parts = lines[i].replace(';', '').replace('(', '').replace(')', '').split()
                    ...
                    Uz = float(parts[-1])
                    magU=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    U_bx01=np.full(nbx01,magU,dtype=np.float64)
                    break
                i += 1
        i +=1
        if (found_ipx01==1):
            ...
            while (i<len(lines)):
                if 'value' in lines[i] and 'nonuniform' in lines[i]:
                    ...
                    if (uniform_bx10==1):
                        uniform_bx10=0
                        epsVal=eps_bx10[0]
                        eps_bx10=np.full(nbx10,epsVal,dtype=np.float64)
                    start_idx = i + 3
                    U_bx10 = np.empty(nbx10, dtype=np.float64)
                    for j in range(nbx10):
                        vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                        parts = vec_line.split()
                        Ux, Uy, Uz = map(float, parts)
                        U_bx10[j]=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    i = start_idx + nbx10  # skip ahead
                    break
                elif 'value' in lines[i] and 'uniform' in lines[i]:
                    parts = lines[i].replace(';', '').replace('(', '').replace(')', '').split()
                    ...
                    Uz = float(parts[-1])
                    magU=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    U_bx10=np.full(nbx10,magU,dtype=np.float64)
                    break
                i += 1
        i +=1
        if (found_ipx10==1):
            ...
            while (i<len(lines)):
                if 'value' in lines[i] and 'nonuniform' in lines[i]:
                    ...
                    if (uniform_by10==1):
                        uniform_by10=0
                        epsVal=eps_by10[0]
                        eps_by10=np.full(nby10,epsVal,dtype=np.float64)
                    start_idx = i + 3
                    U_by10 = np.empty(nby10, dtype=np.float64)
                    for j in range(nby10):
                        vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                        parts = vec_line.split()
                        Ux, Uy, Uz = map(float, parts)
                        U_by10[j]=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    i = start_idx + nby10  # skip ahead
                    break
                elif 'value' in lines[i] and 'uniform' in lines[i]:
                    parts = lines[i].replace(';', '').replace('(', '').replace(')', '').split()
                    ...
                    Uz = float(parts[-1])
                    magU=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    U_by10=np.full(nby10,magU,dtype=np.float64)
                    break
                i += 1
        i +=1
        if (found_ipy10==1):
            ...
            while (i<len(lines)):
                if 'value' in lines[i] and 'nonuniform' in lines[i]:
                    ...
                    if (uniform_bz10==1):
                        uniform_bz10=0
                        epsVal=eps_bz10[0]
                        eps_bz10=np.full(nbz10,epsVal,dtype=np.float64)
                    start_idx = i + 3
                    U_bz10 = np.empty(nbz10, dtype=np.float64)
                    for j in range(nbz10):
                        vec_line = lines[start_idx + j].strip().strip('()')  # remove surrounding ()
                        parts = vec_line.split()
                        Ux, Uy, Uz = map(float, parts)
                        U_bz10[j]=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    i = start_idx + nbz10  # skip ahead
                    break
                elif 'value' in lines[i] and 'uniform' in lines[i]:
                    parts = lines[i].replace(';', '').replace('(', '').replace(')', '').split()
                    ...
                    Uz = float(parts[-1])
                    magU=np.sqrt(Ux*Ux+Uy*Uy+Uz*Uz)
                    U_bz10=np.full(nbz10,magU,dtype=np.float64)
                    break
                i += 1
        i +=1
        if (found_ipz10==1):
...
if (ipz==NPZ-1) and (NPZ>1):
    Dx_bz01=np.empty(nbz01, dtype=np.float64)
    Dy_bz01=np.empty(nbz01, dtype=np.float64)
    Dz_bz01=np.empty(nbz01, dtype=np.float64)
    for j in range(nbz01):
        epsVal=eps_bz01[j]
        magU=U_bz01[j]
        lower = upper = None
        ...
        if (Pe<1):
            Dx_bz01[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha1xVal))
            Dy_bz01[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha1yVal))
            Dz_bz01[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha1zVal))
        else:
            Dx_bz01[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha2xVal))
            Dy_bz01[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha2yVal))
            Dz_bz01[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha2zVal))
...
if (ipy==NPY-1) and (NPY>1):
    Dx_by01=np.empty(nby01, dtype=np.float64)
    Dy_by01=np.empty(nby01, dtype=np.float64)
    Dz_by01=np.empty(nby01, dtype=np.float64)
    for j in range(nby01):
        epsVal=eps_by01[j]
        magU=U_by01[j]
        lower = upper = None
        ...
        if (Pe<1):
            Dx_by01[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha1xVal))
            Dy_by01[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha1yVal))
            Dz_by01[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha1zVal))
        else:
            Dx_by01[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha2xVal))
            Dy_by01[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha2yVal))
            Dz_by01[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha2zVal))
...
if (ipx==NPX-1) and (NPX>1):
    Dx_bx01=np.empty(nbx01, dtype=np.float64)
    Dy_bx01=np.empty(nbx01, dtype=np.float64)
    Dz_bx01=np.empty(nbx01, dtype=np.float64)
    for j in range(nbx01):
        epsVal=eps_bx01[j]
        magU=U_bx01[j]
        lower = upper = None
        ...
        if (Pe<1):
            Dx_bx01[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha1xVal))
            Dy_bx01[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha1yVal))
            Dz_bx01[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha1zVal))
        else:
            Dx_bx01[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha2xVal))
            Dy_bx01[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha2yVal))
            Dz_bx01[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha2zVal))
...
if (ipx==0) and (NPX>1):
    Dx_bx10=np.empty(nbx10, dtype=np.float64)
    Dy_bx10=np.empty(nbx10, dtype=np.float64)
    Dz_bx10=np.empty(nbx10, dtype=np.float64)
    for j in range(nbx10):
        epsVal=eps_bx10[j]
        magU=U_bx10[j]
        lower = upper = None
        ...
        if (Pe<1):
            Dx_bx10[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha1xVal))
            Dy_bx10[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha1yVal))
            Dz_bx10[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha1zVal))
        else:
            Dx_bx10[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha2xVal))
            Dy_bx10[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha2yVal))
            Dz_bx10[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha2zVal))
...
if (ipy==0) and (NPY>1):
    Dx_by10=np.empty(nby10, dtype=np.float64)
    Dy_by10=np.empty(nby10, dtype=np.float64)
    Dz_by10=np.empty(nby10, dtype=np.float64)
    for j in range(nby10):
        epsVal=eps_by10[j]
        magU=U_by10[j]
        lower = upper = None
        ...
        if (Pe<1):
            Dx_by10[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha1xVal))
            Dy_by10[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha1yVal))
            Dz_by10[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha1zVal))
        else:
            Dx_by10[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha2xVal))
            Dy_by10[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha2yVal))
            Dz_by10[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha2zVal))
...
if (ipz==0) and (NPZ>1):
    Dx_bz10=np.empty(nbz10, dtype=np.float64)
    Dy_bz10=np.empty(nbz10, dtype=np.float64)
    Dz_bz10=np.empty(nbz10, dtype=np.float64)
    for j in range(nbz10):
        epsVal=eps_bz10[j]
        magU=U_bz10[j]
        lower = upper = None
        ...
        if (Pe<1):
            Dx_bz10[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha1xVal))
            Dy_bz10[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha1yVal))
            Dz_bz10[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha1zVal))
        else:
            Dx_bz10[j] = Diff*tinv*(1+betaxVal*pow(Pe,alpha2xVal))
            Dy_bz10[j] = Diff*tinv*(1+betayVal*pow(Pe,alpha2yVal))
            Dz_bz10[j] = Diff*tinv*(1+betazVal*pow(Pe,alpha2zVal))
...
if (ipz==NPZ-1) and (NPZ>2):
  ...
  if (uniform_bz01):
      sci_str = format(Dx_bz01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dx_string = mantissa + 'e' + exponent
      sci_str = format(Dy_bz01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dy_string = mantissa + 'e' + exponent
      sci_str = format(Dz_bz01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dz_string = mantissa + 'e' + exponent
      ...
      for j in range(nbz01):
          sci_str = format(Dx_bz01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dx_string = mantissa + 'e' + exponent
          sci_str = format(Dy_bz01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dy_string = mantissa + 'e' + exponent
          sci_str = format(Dz_bz01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dz_string = mantissa + 'e' + exponent
          data.append(f"({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string})\n")
...
if (ipy==NPY-1) and (NPY>2):
  ...
  if (uniform_by01):
      sci_str = format(Dx_by01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dx_string = mantissa + 'e' + exponent
      sci_str = format(Dy_by01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dy_string = mantissa + 'e' + exponent
      sci_str = format(Dz_by01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dz_string = mantissa + 'e' + exponent
      data.append(f"value uniform ({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string});\n")
      ...
      for j in range(nby01):
          sci_str = format(Dx_by01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dx_string = mantissa + 'e' + exponent
          sci_str = format(Dy_by01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dy_string = mantissa + 'e' + exponent
          sci_str = format(Dz_by01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dz_string = mantissa + 'e' + exponent
          data.append(f"({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string})\n")
...
if (ipx==NPX-1) and (NPX>2):
  ...
  if (uniform_bx01):
      sci_str = format(Dx_bx01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dx_string = mantissa + 'e' + exponent
      sci_str = format(Dy_bx01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dy_string = mantissa + 'e' + exponent
      sci_str = format(Dz_bx01[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dz_string = mantissa + 'e' + exponent
      data.append(f"value uniform ({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string});\n")
      ...
      for j in range(nbx01):
          sci_str = format(Dx_bx01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dx_string = mantissa + 'e' + exponent
          sci_str = format(Dy_bx01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dy_string = mantissa + 'e' + exponent
          sci_str = format(Dz_bx01[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dz_string = mantissa + 'e' + exponent
          data.append(f"({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string})\n")
...
if (ipx==0) and (NPX>1):
  ...
  if (uniform_bx10):
      sci_str = format(Dx_bx10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dx_string = mantissa + 'e' + exponent
      sci_str = format(Dy_bx10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dy_string = mantissa + 'e' + exponent
      sci_str = format(Dz_bx10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dz_string = mantissa + 'e' + exponent
      data.append(f"value uniform ({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string});\n")
      ...
      for j in range(nbx10):
          sci_str = format(Dx_bx10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dx_string = mantissa + 'e' + exponent
          sci_str = format(Dy_bx10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dy_string = mantissa + 'e' + exponent
          sci_str = format(Dz_bx10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dz_string = mantissa + 'e' + exponent
          data.append(f"({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string})\n")
...
if (ipy==0) and (NPY>1):
  ...
  if (uniform_by10):
      sci_str = format(Dx_by10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dx_string = mantissa + 'e' + exponent
      sci_str = format(Dy_by10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dy_string = mantissa + 'e' + exponent
      sci_str = format(Dz_by10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dz_string = mantissa + 'e' + exponent
      data.append(f"value uniform ({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string});\n")
      ...
      for j in range(nby10):
          sci_str = format(Dx_by10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dx_string = mantissa + 'e' + exponent
          sci_str = format(Dy_by10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dy_string = mantissa + 'e' + exponent
          sci_str = format(Dz_by10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dz_string = mantissa + 'e' + exponent
          data.append(f"({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string})\n")
...
if (ipz==0) and (NPZ>1):
  ...
  if (uniform_bz10):
      sci_str = format(Dx_bz10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dx_string = mantissa + 'e' + exponent
      sci_str = format(Dy_bz10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dy_string = mantissa + 'e' + exponent
      sci_str = format(Dz_bz10[0], ".7e")
      mantissa, exponent = sci_str.split('e')
      mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
      Dz_string = mantissa + 'e' + exponent
      data.append(f"value uniform ({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string});\n")
      ...
      for j in range(nbz10):
          sci_str = format(Dx_bz10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dx_string = mantissa + 'e' + exponent
          sci_str = format(Dy_bz10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dy_string = mantissa + 'e' + exponent
          sci_str = format(Dz_bz10[j], ".7e")
          mantissa, exponent = sci_str.split('e')
          mantissa = mantissa.rstrip('0').rstrip('.')  # Also remove trailing dot if all decimals are stripped
          Dz_string = mantissa + 'e' + exponent
          data.append(f"({Dx_string} 0 0 0 {Dy_string} 0 0 0 {Dz_string})\n")
